...
public sealed class MetadataIndex : IDisposable
{
    ...
    private static readonly System.Collections.Concurrent.ConcurrentDictionary<string,string> NormalNames=new();
    public static string Name(string name)=>!Obfuscated(name)?name:NormalNames.GetOrAdd(name,n=>Regex.Replace(n,@"[\u0370-\u1fff]+","?"));
