        ...
        foreach(var expected in contract.Types)
        {
            ...
            else
            {
                ...
                if(candidates.Length==0)
                {
                    ...
                    if(candidates.Length==0&&expected.Members.Length>=2)
                    {
                        // Large managers change unrelated methods frequently. Anchor the actual
                        // interfaces we use, with two independent body/call-site matches.
                        var required=expected.Members.Select(m=>m.Signature).Distinct().ToArray();
                        var interfaces=index.Types.Where(t=>MetadataIndex.Obfuscated(t.FullName))
                            .Select(t=>(Type:t,Members:MetadataIndex.Members(t).ToArray()))
                            .Where(x=>required.All(s=>x.Members.Any(m=>MetadataIndex.Signature(m)==s)))
                            .Select(x=>(x.Type,Score:expected.Members.Count(e=>x.Members.Any(m=>MetadataIndex.Signature(m)==e.Signature&&
                                (e.Body.Length>0&&index.MemberBody(m)==e.Body || index.Uses(m).Intersect(e.Uses).Any())))))
                            .Where(x=>x.Score>=2).OrderByDescending(x=>x.Score).ToArray();
                        if(interfaces.Length>0)candidates=interfaces.Where(x=>x.Score==interfaces[0].Score).Select(x=>x.Type).ToArray();
                    }
                }
            }
