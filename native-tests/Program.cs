...
if(regression){CatalogChecks.Run();CompatibilityChecks.Run();TableReaderChecks.Run();UiFlowChecks.Run();PresentationExecutionChecks.Run();Console.WriteLine(new JsonObject{["checks"]=testIndex,["status"]=failures.Count==0?"passed":"failed",["failures"]=new JsonArray(failures.Select(x=>(JsonNode?)JsonValue.Create(x)).ToArray())}.ToJsonString());if(failures.Count>0)Environment.ExitCode=1;}
